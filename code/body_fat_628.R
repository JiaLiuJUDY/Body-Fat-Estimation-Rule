...
(zero <- which(BODYFAT == 0)) # 182
(large <- which(BODYFAT == max(BODYFAT))) # 216
...
data[which(WEIGHT > 148 & WEIGHT < 149), ] # compare to 24. the measurement of YEAR NECK CHEST ABDOMEN  HIP for 76 are larger than 24, it is impossible for him to have a smaller body fat!!! keep body fat !!!

### check 48
495/DENSITY[48]- 450 # 14.13502
data[which(WEIGHT > 148 & WEIGHT < 149), ] # compare to 24. All of his measurement are quite similar to 24, it is impossible for his to have so small body fat, keep density!!!
...
bad <- c(bad, 221, 163) # can not fix the records, since we do not know which measurement is wrong >>> exclude them
...
# RIDGE, ACTUALLY not to select vars
set.seed(2018)
...
(s2 <- summary(m2))
(mse2 <- sum((s2$residuals)^2)/nrow(data)) # 16.3409
...
res <- fit - BODYFAT
summary(res)
fit <- -37.5 + ABDOMEN - 0.2*WEIGHT 
(mse <- sum((fit - BODYFAT)^2)/nrow(data)) # good!

m3 <- lm(BODYFAT ~ ABDOMEN)
s3 <- summary(m3)
(mse3 <- sum((s3$residuals)^2)/nrow(data)) # 19.95197 close enough to m2

m4 <- lm((BODYFAT)*WEIGHT ~ ABDOMEN + WEIGHT, data) # transform
summary(m4)
(mse4 <- sum((m4$residuals/WEIGHT)^2)/nrow(data)) # 16.45531 worse than m2 >>> PASS

m5 <- lm(BODYFAT ~ WEIGHT + ABDOMEN + FOREARM + WRIST, data) # WEIGHT, ABDOMEN, FOREARM, WRIST  
(s5 <- summary(m5))
(mse5 <- sum((s5$residuals)^2)/nrow(data)) # although better, too many vars
# round the model
crPlot(m5)
